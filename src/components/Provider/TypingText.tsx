...
function SmoothEffect({ words, index, alwaysVisibleCount }: { words: string[]; index: number; alwaysVisibleCount: number }) {
   return (
...
function NormalEffect({ text, index, alwaysVisibleCount }: { text: string; index: number; alwaysVisibleCount: number }) {
   ...
   return (
      <div className={cn("relative font-mono", className)}>
         ...
         >
            ...
            <CursorWrapper waiting={waitingNextCycle} visible={Boolean(!smooth && cursor && (!hideCursorOnComplete || !isComplete))}>
               {cursor}
            </CursorWrapper>
         </div>
      </div>
